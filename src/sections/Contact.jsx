...
import { SectionHeading, Card, Icon, CheckList, Input, Button } from '../components/index.js';
...
function ServiceSelect({ value, onChange }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
      <label htmlFor="svc" style={{ fontSize: 'var(--text-sm)', fontWeight: 500, color: 'var(--text-body)' }}>
        What do you need?
      </label>
      <select
        id="svc"
...
export function Contact() {
  ...
  const [form, setForm] = useState({ name: '', business: '', email: '', phone: '', service: SERVICE_OPTIONS[0], message: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    setError(false);
    try {
      await submitForm({ form: 'contact', ...form });
      setSent(true);
    } catch (err) {
  ...
  return (
    <Section id="contact">
      <SectionHeading
        eyebrow="Contact"
        title="Tell me what&rsquo;s going on with your books"
          ...
          <Card tone="paper">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
              <a href={`tel:${BRAND.phone.replace(/\D/g, '')}`} style={{ display: 'flex', gap: 12, alignItems: 'center', textDecoration: 'none', color: 'var(--text-body)' }}>
                ...
                <span>
                  <span style={{ display: 'block', fontSize: 'var(--text-xs)', color: 'var(--text-subtle)' }}>Phone</span>
                  <span style={{ fontSize: 'var(--text-base)', fontWeight: 500 }}>{BRAND.phone}</span>
                </span>
              </a>
              <a href={`mailto:${BRAND.email}`} style={{ display: 'flex', gap: 12, alignItems: 'center', textDecoration: 'none', color: 'var(--text-body)' }}>
                ...
                <span>
                  <span style={{ display: 'block', fontSize: 'var(--text-xs)', color: 'var(--text-subtle)' }}>Email</span>
                  <span style={{ fontSize: 'var(--text-base)', fontWeight: 500, wordBreak: 'break-all' }}>{BRAND.email}</span>
                </span>
              </a>
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                <Icon name="map-pin" size={18} color="var(--action-primary)" />
                <span>
                  <span style={{ display: 'block', fontSize: 'var(--text-xs)', color: 'var(--text-subtle)' }}>Service area</span>
                  <span style={{ fontSize: 'var(--text-base)', fontWeight: 500 }}>{BRAND.area}</span>
                </span>
              </div>
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                <Icon name="clock" size={18} color="var(--action-primary)" />
                <span>
                  <span style={{ display: 'block', fontSize: 'var(--text-xs)', color: 'var(--text-subtle)' }}>Hours</span>
                  <span style={{ fontSize: 'var(--text-base)', fontWeight: 500 }}>{BRAND.hours}</span>
                </span>
              </div>
            </div>
          </Card>
          ...
          {sent ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', alignItems: 'flex-start' }}>
              ...
              <p style={{ margin: 0, color: 'var(--text-muted)' }}>
                I&rsquo;ll read it today and come back with two questions and a couple of times to talk. If it&rsquo;s urgent, call {BRAND.phone}.
              </p>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
              ...
              {error ? (
                <p style={{ margin: 0, fontSize: 'var(--text-sm)', color: 'var(--danger-600)' }}>
                  Something went wrong sending that — please try again, or email {BRAND.email} directly.
                </p>
              ) : null}
            </form>
          )}
